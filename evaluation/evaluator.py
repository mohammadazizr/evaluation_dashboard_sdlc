...
class RAGEvaluatorNew:
    ...
    def __init__(self, ground_truth_path: str, k: int = 10, confidence_threshold: float = 0.7):
        ...
        self.results = defaultdict(lambda: {
            ...
            'precisions': [],
            'precision_at_3': [],  # Track precision@3 metric
            'total_questions': 0,
            'successes': 0
        })
        ...
        # Hierarchical metrics storage (by project, activity, doc_type)
    ...
    def normalize_path(self, path: str) -> str:
        ...
        prefixes_to_remove = [
            ...
            'qdrant\\mpm\\',
            'mpm\\',
            'd:\\simplify\\rag-sdlc\\output\\ocr_result\\operation request portal\\',
            'd:\\simplify\\rag-sdlc\\output\\ocr_result\\workflow management system\\'
        ]
        ...
        # 3. String replacements untuk normalisasi nama folder
        replacements = {
    ...
    def extract_hierarchy_from_path(self, normalized_path: str) -> Dict[str, str]:
        """
        Extract project, activity, and doc_type from normalized path
        Expected format: project\\activity\\doc_type\\filename.md
        """
    ...
    def evaluate_retrieval(self,
        ...
        if test_type == 'no_doc':
            ...
            # Calculate precision@3: 1 if any correct document found in top-3, else 0
            precision_at_3 = 1.0 if any(pos <= 3 for pos in positions_found) else 0.0
            ...
            result['precision_at_3'] = precision_at_3
            ...
            test_results['precision_at_3'].append(precision_at_3)
            ...
            # Update hierarchical metrics based on expected documents
    ...
    def evaluate_all(self, retrieval_results: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        hierarchical_metrics = self.calculate_hierarchical_metrics()
    ...
    def calculate_aggregate_metrics(self) -> Dict[str, Any]:
        ...
        for test_type in retrieval_types:
            ...
            if test_results['total_questions'] > 0:
                ...
                avg_precision_at_3 = sum(test_results['precision_at_3']) / len(test_results['precision_at_3'])
                ...
                metrics[test_type] = {
                    ...
                    'precision_at_3': round(avg_precision_at_3, 4),
        ...
        for test_type in retrieval_types:
            ...
            all_precision_at_3.extend(self.results[test_type]['precision_at_3'])
        ...
        if total_retrieval_q > 0:
            metrics['overall'] = {
                ...
                'precision_at_3': round(sum(all_precision_at_3) / len(all_precision_at_3), 4),
                'success_rate': round(total_successes / total_retrieval_q, 4)
            }

        return metrics

    def calculate_hierarchical_metrics(self) -> Dict[str, Any]:
        """Calculate metrics organized by project, activity, and doc_type"""
    ...
    def print_summary(self, evaluation_results: Dict[str, Any]):
        ...
        print(f"   [OK] Evaluated: {summary.get('questions_evaluated', 0)}")
        print(f"   [WARNING] Missing: {summary.get('questions_missing', 0)}")
...
if __name__ == "__main__":
    ...
    from config import GROUND_TRUTH_PATH, RETRIEVAL_RESULTS_RERANKED, EVALUATION_RESULTS_PATH
    ...
    RETRIEVAL_RESULTS_FILE = str(RETRIEVAL_RESULTS_RERANKED)
    OUTPUT_FILE = str(EVALUATION_RESULTS_PATH)

    print(f"Ground Truth File: {GROUND_TRUTH_FILE}")
    print(f"Retrieval Results File: {RETRIEVAL_RESULTS_FILE}")
    # print(f"Evaluation Output File: {OUTPUT_FILE}")
    if len(sys.argv) > 1:
    ...
    print(f"\n[INFO] Loading retrieval results...")
    ...
    if evaluator.missing_questions:
        print(f"\n[WARNING] {len(evaluator.missing_questions)} questions not evaluated")
