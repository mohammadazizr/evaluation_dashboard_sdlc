...
class RAGPipeline:
    ...
    def run_stage(self, stage_num: int, stage_name: str, script_path: str, working_dir: str) -> bool:
        ...
        print(f"[DIR] Working directory: {full_working_dir}")
        print(f"[SCRIPT] Script: {full_script_path}")
        ...
        try:
            ...
            print(f"[ERROR] {stage_name} timed out after 10 minutes")
            self.results[stage_name] = {
                'status': 'timeout',
                'timestamp': datetime.now().isoformat()
            }
            ...
            print(f"[ERROR] Error running {stage_name}: {str(e)}")
            self.results[stage_name] = {
                'status': 'error',
                'error': str(e),
                'timestamp': datetime.now().isoformat()
    ...
    def run_pipeline(self):
        ...
        print(f"   Failed: {len(failed_stages)} [ERROR]")
        print(f"\n[TIME] Execution time: {duration.total_seconds():.2f} seconds")
        print(f"   Started: {self.start_time.strftime('%H:%M:%S')}")
        print(f"   Ended: {end_time.strftime('%H:%M:%S')}")

        if failed_stages:
            print(f"\n[WARNING] Failed stages:")
